...
from `covid-19-analysis-450007.covid19_recovered.recovered`
group by Country_Region
order by Country_Region

--- check for nules
----cumilative recovered case for each country in 2020
SELECT Country_Region, 
   SUM(january + February+march+april+may+june+july+august+september+october+november+december) AS total_recovered_2020
FROM `covid-19-analysis-450007.covid19_recovered.recovery_clean`
group by Country_Region
order by total_recovered_2020 desc


---total recovered cases globally in 2020
select 
    SUM(january + February+march+april+may+june+july+august+september+october+november+december) AS total_death_2020
from `covid-19-analysis-450007.covid19_recovered.recovery_clean`

--- total recovered globally every month in 2020
...
FROM `covid-19-analysis-450007.covid19_recovered.recovery_clean`
group by Country_Region
order by total_death_2020 desc
limit 10

--- countries with more than 0 recoveries in january 
select Country_Region, january
from `covid-19-analysis-450007.covid19_recovered.recovery_clean`
where january >0
order by january desc

--- countries with 0 recoveries in decemeber
...
--- average numbeer of recoveries per country per month
